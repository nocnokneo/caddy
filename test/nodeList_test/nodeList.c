...
// Returns true when node1 == node2
BOOL equals( NODE *node1, NODE *node2 )
{
   int i;
   
   if( node1->numAdjNodes != node2->numAdjNodes )
   {
      return FALSE;
   }
   
   //lcdWriteStr("sz1:",1,0);
   //lcdPrintHex(node1->numAdjNodes,1,5);
   //lcdWriteStr("sz2:",1,8);
   //lcdPrintHex(node2->numAdjNodes,1,13);

   
   for( i = 0; i < node1->numAdjNodes; i++ )
   {
      if( node1->adjNodes[i] != node2->adjNodes[i] ||
          node1->adjCosts[i] != node2->adjCosts[i] || 
          node1->adjHeadings[i] != node2->adjHeadings[i] )
      {
         return FALSE;
      }
   }
...
int main(void)
{
   ...
   {
      getNode( i, &node );
      if( !equals( &nodeList[i], &node ) )
      {
         lcdWriteStr("Failed, i = ",0,0);
         lcdPrintDecU08(i,0,12);
         while(1) ;
      }
   }
   lcdWriteStr("Success",0,0);
   //*/
   
   /*
   getNode( 1, &node );
   if( equals( &nodeList[1], &node ) )
   {
      lcdWriteStr("Equal",0,0);
   }
   else
   {
      lcdWriteStr("Not equal",0,0);
   }
   */
