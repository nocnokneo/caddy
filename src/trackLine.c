...
void trackLineInit(void)
{
   ...
   rprintf("tc %d %d %d %d %d %d\r", LINE_RMIN, LINE_RMAX, LINE_GMIN, 
...
void analyzeLineStats(void) 
{
   ...
   correction = slopeCoef * m + offCoef*(lineCenter - (m * (LINE_Y3 - lineY1) + lineX1)) +
                dampCoef * damping;   
   ...
   if( lcdMode == LINE_LCD_MODE )
   {
      ...
      /*
      lcdPrintHex(correction>>8, 1, 9);
      lcdPrintHex(correction, 1, 11);
      */
      ...
      //static u08 packetsAnalyzed = 0;
      //lcdWriteStr("p", 0, 5);
      //lcdPrintHex(packetsAnalyzed++, 0, 6);
   }
#endif
...
/*
 * Adjusts each motor's pulse width depending on where camera sees line.
 * (Called by moveToNextJunction in botCntrl.c)
 *   PRE: lineStats buffer needs to be processed
 */
void printPacket(void)
{
      ...
      {
         /*
         lcdPrintHex(lineStats[0][X_MEAN], 0, 0);
         lcdPrintHex(lineStats[0][X_MIN], 0, 3);
         lcdPrintHex(lineStats[0][X_MAX], 0, 6);
         lcdPrintHex(lineStats[0][LINE_COUNT], 0, 9);
         lcdPrintHex(lineStats[1][X_MEAN], 1, 0);
         lcdPrintHex(lineStats[1][X_MIN], 1, 3);
         lcdPrintHex(lineStats[1][X_MAX], 1, 6);
         lcdPrintHex(lineStats[1][LINE_COUNT], 1, 9);
         */
         
         /*
         lcdPrintHex(lineStats[0x21][X_MEAN], 0, 5);
         lcdPrintHex(lineStats[0x21][X_MIN], 0, 8);
         lcdPrintHex(lineStats[0x21][X_MAX], 0, 11);
         lcdPrintHex(lineStats[0x21][LINE_COUNT], 0, 14);
         lcdPrintHex(lineStats[0x22][X_MEAN], 1, 5);
         lcdPrintHex(lineStats[0x22][X_MIN], 1, 8);
         lcdPrintHex(lineStats[0x22][X_MAX], 1, 11);
         lcdPrintHex(lineStats[0x22][LINE_COUNT], 1, 14);
         */
      }
#endif
